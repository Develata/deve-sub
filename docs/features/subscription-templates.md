...
成员在生成时检查，节点失效会明确报错。重名节点、与代理组/内建策略同名的
节点会使用稳定的 ID 后缀输出，节点池中的显示名称保持原样。

保存会检查基本结构、常见 CIDR/NETWORK 参数、策略/规则集引用、重名与循环。
客户端特定正则、DNS/TUN 细节仍以实际 Mihomo 校验为准，不把服务端的检查
当作完整客户端 schema。高级选项还取决于客户端版本，例如 inline 规则集
