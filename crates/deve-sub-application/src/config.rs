...
pub struct AppConfig {
    ...
    /// Audit history retention. Runtime output is managed by the deployment.
    #[serde(default)]
...
impl AppConfig {
    ...
    pub fn validate(&self) -> Vec<ValidationIssue> {
        ...
        if self.logging.audit_retention_days > 3650 {
            issues.push(ValidationIssue {
                severity: IssueSeverity::Error,
                message: "logging.audit_retention_days must be 0 (disabled) or 1–3650".into(),
            });
        }
