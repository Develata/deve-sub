...
<p align="center">
  <a href="https://github.com/Develata/deve-sub/releases/tag/v0.1.3">下载 v0.1.3</a> ·
...
**当前稳定版：[`v0.1.3`](https://github.com/Develata/deve-sub/releases/tag/v0.1.3)。**
...
curl -fsSL --connect-timeout 15 --max-time 60 \
  https://raw.githubusercontent.com/Develata/deve-sub/v0.1.3/scripts/install.sh \
  -o install-deve-sub-v0.1.3.sh
```

审阅 `install-deve-sub-v0.1.3.sh` 后执行；先让服务只监听本机：

```bash
sudo env DEVE_SUB_VERSION=v0.1.3 DEVE_SUB_BIND=127.0.0.1:8080 \
  sh install-deve-sub-v0.1.3.sh
```
...
下载 [`v0.1.3` 随附的 Compose 文件](https://raw.githubusercontent.com/Develata/deve-sub/v0.1.3/docker-compose.yml)，
...
curl -fsSL --connect-timeout 15 --max-time 60 \
  https://raw.githubusercontent.com/Develata/deve-sub/v0.1.3/docker-compose.yml \
  -o docker-compose.yml
printf 'DEVE_SUB_IMAGE_TAG=v0.1.3\n' > .env
chmod 600 .env
...
`v0.1.3` 的 `--force` 仅允许同版本重装，降级另需 `--allow-downgrade`。这些参数不关闭签名验证。
