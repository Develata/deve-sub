//! VMess AEAD KDF — recursively-nested HMAC-SHA256.
//!
//! Mirrors v2ray `proxy/vmess/aead/kdf.go`. With a single path segment this
//! is plain HMAC-SHA256; each additional segment wraps another HMAC whose
//! underlying hash primitive is the *inner HMAC* (not raw SHA-256).
...
mod tests {
    ...
    fn kdf_matches_reference_vectors() {
        assert_eq!(
            kdf(b"key", &[b"label"]),
            hex32("c9cebf77e859ffcbe78619d4e503b0df707f1d7ac98a189c418763940880e3eb")
        );
        assert_eq!(
            kdf(
            ...
            hex32("53e9d7e1bd7bd25022b71ead07d8a596efc8a845c7888652fd684b4903dc8892")
        );
    }
